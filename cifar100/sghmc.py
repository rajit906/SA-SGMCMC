#!/usr/bin/env python3
"""
SGHMC CIFAR100 Training with metrics logging.
"""
...
# CLI arguments
def parse_args():
    parser = argparse.ArgumentParser(description='SGHMC CIFAR100 Training')
    parser.add_argument('--dir', default='cifar100/ckpt_sghmc', help='path to save checkpoints')
    ...
    parser.add_argument('--sampler_type', required=True, choices=['sghmc', 'csghmc', 'sa-sghmc', 'sa-csghmc'])
    parser.add_argument('--momentum', type=float, default=0.9, help='1: SGLD; <1: SGHMC')
    ### These are for SA-SGHMC
    parser.add_argument('--alpha', default=50., type=float)
    parser.add_argument('--lr_init', default=0.01, type=float)  # This is for SGHMC as well
...
if __name__ == "__main__":
    ...
    if args.sampler_type == 'csghmc':
        num_cycles = 4
        lr_0 = 0.5
    
    if args.sampler_type == 'sa-csghmc':
        num_cycles = 4
        lr_0 = 0.05
    ...
    def adjust_learning_rate(epoch, batch_idx, gradnorm, zeta):
        ...
        elif args.sampler_type == 'sa-csghmc':
            cos_inner = np.pi * (rcounter % (T // num_cycles)) / (T // num_cycles)
    ...
    # SGHMC update with momentum buffer
    def update_params(lr, epoch):
        for p in net.parameters():
            if not hasattr(p, 'buf'):
                p.buf = torch.zeros_like(p.data)
            d_p = p.grad.data + weight_decay * p.data
            buf = (1-args.momentum) * p.buf - lr * d_p
            if ((args.sampler_type in ['sa-sghmc', 'sghmc']) and epoch >= 150) or \
                ((args.sampler_type in ['csghmc', 'sa-csghmc']) and ((epoch % 50) + 1 > 45)):
                noise = torch.randn_like(p.data) * np.sqrt(2*lr*args.momentum*args.temperature/datasize)
                buf += noise
            p.data.add_(buf)
            p.buf = buf
    ...
    def train(epoch, zeta):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            gradnorm = 0.
            if args.sampler_type == 'sa-sghmc' or args.sampler_type == 'sa-csghmc':
